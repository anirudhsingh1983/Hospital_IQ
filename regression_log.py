...
import functools, requests, logging, logging.config 
...
def read_from_API(constants,logger):
    """
    ...
    try:
        ...
        logger.info("API read successful.")
...
def main():
    ...
    fh = logging.FileHandler(filename =  'logs/regression.py_'+ datetime.now().strftime("%d_%m_%Y_%H_%M_%S") + '.log', mode='w')
    ...
    data,read_success = read_from_API(constants=CONSTANTS,logger=logger)
    if read_success == True: 
        ...
        logger.info('Dataset splitted to train and test')
